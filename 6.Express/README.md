...
1. 서버는 https://www.naver.com 의 주소로 GET 방식의 **Request**가 온 것을 확인하고 그에 해당하는 html 파일을 응답(**Response**)로 보내준다.  
...
Headers에는 HTTP 통신을 할 때 이 **request**가 어떤 **request**인지, 이 **response**가 어떤 **response**인지 간략한 설명이 담겨 있습니다.  
...
그렇지만 개발자 도구의 네트워크탭을 보면 약 99%의 **GET**과 약 1%의 **POST**로 구성되어 있는 것을 쉽게 찾을 수 있을 것입니다.  

그래서 **GET**과 **POST**만 간략히 설명해 드리려고 합니다.  
...
여러분이 경험해 보셨을 만한 **POST**는 온라인 결제창에서 새로고침을 누르면 양식 다시 제출 확인이라는 문구를 보셨을 겁니다.  
...
네이버의 초록색 검색창이 제대로 보이신다면 아마 Status Code에는 `200`이 표시되어 있을것입니다.  
흔히 `404`는 많이 보셨을 텐데요.  
`404`는 존재하지 않는 주소로의 접근입니다.  
많은 Status code가 있지만,  
간단히 정리하자면 200은 아무 문제 없이 정상적으로 http 통신이 완료된 것입니다.  
`300`대의 status code는 **redirection**과 관련된 경우입니다.  
`400`대의 status code는 주로 **클라이언트(사용자)의 잘못**인 경우이며,  
`500`대의 status code는 주로 **서버의 잘못**인 경우입니다.  

Status Code에 대해 더 알아보고 싶으시다면 [다음 문서](https://developer.mozilla.org/ko/docs/Web/HTTP/Status)를 참고해 주세요.  

---

### (2) Npm
**npm**은 Node Package Manager의 약자로 Node.js의 패키지(모듈)의 설치, 삭제 및 업데이트를 도와줍니다.  
...
그래서 사용자에게 필요한 모듈만 선택하여 설치하도록 해주는 것이 바로 **npm**입니다.  
...
현재 폴더의 위치를 이 강의자료가 있는 폴더로 두시고 다음 명령어를 입력해 주세요.
```bash
npm init
```
다양한 입력을 받을텐데 
**Entry point**만 **app.js**로 해 주시고  
나머지는 다 enter를 눌러주시면 됩니다.  
...
### (3) Express
**Express**는 Node.js 기반의 웹 프레임워크입니다.  
...
소스코드를 설명하자면  
`require()`를 통해 express 모듈을 불러온 후,  
`express()`를 통해 객체를 하나 생성합니다.  
그 다음으로는 '/' 주소로 **GET** 방식의 **request**가 오면 callback 함수를 실행하여 **response**를 설정하는 `app.get` 함수가 있습니다.  
...
이전 예제와 같이 index.html를 fs 모듈을 통해 읽어 웹 브라우저에 **response**로 보냈습니다.  
localhost:3000 에서
**GET** 버튼을 누르면 /test의 주소로 **GET request**가 전송됩니다.  
**POST** 버튼을 누르면 /test의 주소로 **POST request**가 전송됩니다.  
...
이번에는 **request**를 받고 다른 주소로 **redirection**를 해 줘 봅시다.  
```javascript
//redirection.js
const fs=require('fs');
const express=require('express');
